...
################################################################
#1. Coexistence time ------------------------------------
coexist_summ <- here::here('data/COEXsim_summary_28FEB25.rds')%>%
  readRDS()%>%
  bind_rows(here::here('data/ASSsim_summary_28FEB25.rds')%>%
              readRDS())
...
ass_aux <- here::here('data/ASSsim_28FEB25.rds')%>%
  readRDS()%>%
  filter(shuffle_p == 0)%>%
  filter(aij >= 1)%>%
...
############################################################
# Supplement Fig S1: Full parameter space -----------------
############################################################
figs1 <- summ_data%>%
  ggplot(aes(x = disp_rate, y = delta))+
  geom_tile(aes(fill = time), col = 'black', size = 0.5)+
