...
coexist_dat <- par_space%>%
  ...
  #mutate(norm_MI = I/Hx)%>%
  #mutate(norm_MI = ifelse(is.na(norm_MI), 0, norm_MI))%>%
  mutate(r_freq = r_N1 / (r_N1  + r_N2))%>%
  mutate(total_N = r_N1  + r_N2)

saveRDS(coexist_dat, 'COEXsim_28FEB25.rds')
...
#Export summary
saveRDS(coexist_summ, 'COEXsim_summary_28FEB25.rds')
...
#Save plot
ggsave('coex_28FEB25.png', plot = co_plot, width = 16, height = 8)
